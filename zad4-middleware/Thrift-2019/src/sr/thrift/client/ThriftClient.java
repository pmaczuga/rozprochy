...
public class ThriftClient 
{
	public static void main(String [] args) 
	{
		...
		String opt = "simple"; //simple | multiplex | non-block | asyn | multi-thread
		...
		try {
			...
			{
				transport = new TSocket(host, 9091);
				...
				transport = new TFramedTransport(new TSocket(host, 9093));
				protocol = new TBinaryProtocol(transport);
			}
				...
				try {
					nbtransport = new TNonblockingSocket(host, 9093);
			...
			else if (opt.contains("multi-thread")) { //TODO
				transport = new TSocket(host, 9092);
			}
			else 
			{
