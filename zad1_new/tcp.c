...
struct Token receive_tcp(struct Sockets *sockets)
{
    ...
    {
        char msg[BUF_SIZE];
        int res = recv(event.data.fd, msg, BUF_SIZE, 0);
        if (res < 0) print_error_errno("Problem receiving message");
...
void delete_socket(struct Sockets *sockets, int socket_fd)
{
    int res;

    res = epoll_ctl(sockets->epoll_fd, EPOLL_CTL_DEL, socket_fd, NULL);
    if (res != 0) end_with_error_errno("Problem removing socket from epoll - delete_socket");

    res = shutdown(socket_fd, SHUT_RDWR);
    if (res != 0) end_with_error_errno("Problem shutting down socket - delete_socket");

    res = close(socket_fd);
    if (res != 0) end_with_error_errno("Problem closing socket - delete_socket");
}

void confirm_change_of_input(struct Sockets *sockets, int useful_int)
{
    delete_socket(sockets, sockets->socket_in);
